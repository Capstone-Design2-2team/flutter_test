...
class WalkRecordEditSimpleScreen extends StatefulWidget {
  ...
  @override
  State<WalkRecordEditSimpleScreen> createState() =>
      _WalkRecordEditSimpleScreenState();
}

class _WalkRecordEditSimpleScreenState
    extends State<WalkRecordEditSimpleScreen> {
  ...
  Future<void> _saveWalkRecord() async {
    ...
    try {
      ...
      if (_newImage != null) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('이미지 업로드 기능은 준비 중입니다.')));
      }
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('산책 기록이 수정되었습니다.')));
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('수정에 실패했습니다: ${e.toString()}')));
      }
    } finally {
