...
class WalkFromRecordScreen extends StatefulWidget {
  final VoidCallback onBackToHome;

  const WalkFromRecordScreen({super.key, required this.onBackToHome});

  @override
  State<WalkFromRecordScreen> createState() => _WalkFromRecordScreenState();
}

class _WalkFromRecordScreenState extends State<WalkFromRecordScreen> {
  ...
  Future<void> _startWalk() async {
    ...
    _tick = Timer.periodic(
      ...
      (_) => setState(() => _elapsed = DateTime.now().difference(_startedAt!)),
    );
  }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          onPressed: () {
            Navigator.pop(context); // 산책기록 확인 화면으로 돌아가기
          },
        ),
      ),
      body: Stack(
              ...
              builder: (context, snapshot) {
                ...
                return ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final id = doc.id;
                    final name = (data['name'] ?? data['pet_name'] ?? '펫')
                        .toString();
                    final breed = (data['breed'] ?? data['pet_breed'] ?? '모르겠음')
                        .toString();
                    ...
                    return CheckboxListTile(
                      ...
                      onChanged: (checked) {
                        if (checked == true) {
                          setState(() => _selectedPetIds.add(id));
                        } else {
                          setState(() => _selectedPetIds.remove(id));
                        }
                      },
                    );
                  },
