...
class _ActivityHistoryScreenState extends State<ActivityHistoryScreen> {
  ...
  Future<void> _loadActivityHistory() async {
    try {
      ...
      for (var doc in allWalks.docs) {
        final data = doc.data();
        print(
          'All walk record - ID: ${doc.id}, user_id: ${data['user_id']}, current_user: ${user.uid}',
        );
      }
        ...
        if (data != null) {
          ...
          activities.add({
            ...
            'distance': data['distance_km'] != null
                ? '${(data['distance_km'] as double).toStringAsFixed(2)}km'
                : null,
            'duration': data['duration_minutes'] != null
                ? '${data['duration_minutes']}분'
                : null,
            'createdAt': walkDate,
          });
        }
  ...
  Widget _buildActivityCard(Map<String, dynamic> activity) {
    return InkWell(
      onTap: () {
        if (activity['type'] == 'walk' && activity['id'] != null) {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => WalkRecordDetailScreen(recordId: activity['id']),
            ),
          );
        }
      },
